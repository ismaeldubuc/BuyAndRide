          ...
          <button
            onClick={async () => {
              try {
                ...
                if (response.ok) {
                  navigate("/addVehicule"); // L'utilisateur est authentifié
                } else {
                  alert("Veuillez vous connecter pour accéder à cette page.");
                  navigate("/login-page"); // Redirection si non authentifié
                }
              } catch (error) {
                console.error(
        ...
        <div className="relative">
          ...
          {showMenu && (
            <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg py-2 z-50 shadow-[0_0_10px_rgba(0,0,0,0.2)]">
              <button className="w-full px-4 py-2 text-left text-[#24507F] hover:bg-gray-100 transition duration-300">
                Profil
              </button>
              <button className="w-full px-4 py-2 text-left text-[#24507F] hover:bg-gray-100 transition duration-300">
                Déconnexion
